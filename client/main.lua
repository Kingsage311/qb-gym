...
CreateThread(function()
    while true do
        ...
        for k in pairs(Config.Locations) do
            local dist = #
                (pos - vector3(Config.Locations[k].coords.x, Config.Locations[k].coords.y, Config.Locations[k].coords.z)
                )
            if dist < 4.5 then
                if dist < Config.Locations[k].viewDistance then
                    sleep = 0
                    DrawText3D(Config.Locations[k].coords.x, Config.Locations[k].coords.y, Config.Locations[k].coords.z,
                        Config.Locations[k].Text3D)
                    if IsControlJustReleased(0, 38) then
                        QBCore.Functions.Notify('Preparing exercise', 'success')
                        Wait(1000)
                        SetEntityHeading(ped, Config.Locations[k].heading)
                        SetEntityCoords(ped, Config.Locations[k].coords.x, Config.Locations[k].coords.y,
                            Config.Locations[k].coords.z - 1)
                        ...
                        QBCore.Functions.Notify('You need to rest ' ..
                            Config.RestingTime .. ' more seconds before doing another exercise', 'error')
                        training = true
                        resting = true
                        CheckTraining()
